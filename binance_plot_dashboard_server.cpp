...
static std::vector<TimingRow> read_timing_rows(const std::string& market_raw, const std::string& symbol_raw, int max_rows) {
    ...
    for (size_t i = 0; i < header_fields.size(); ++i) h[lower(trim(header_fields[i]))] = i;
    ...
    while (std::getline(f, line)) {
        ...
        if (row_market != market || row_symbol != symbol) continue;
...
async function loadMarkets(){
  ...
  $('rootStatus').textContent=`BASE_DIR: ${data.base_dir} | CSV: ${data.timing_csv_path}`;
...
async function loadSymbols(){
  ...
  if(!data.symbols.length){ $('rangeStatus').innerHTML=`<span class="bad">No symbols found for ${market}</span>`; Plotly.purge('ohlcvChart'); Plotly.purge('timingChart'); renderTable('summaryTable',[]); return; }
...
async function plotOHLCV(){
  const market=$('market').value, symbol=$('symbol').value, bars=$('bars').value || 500, plotType=$('plotType').value;
  const data=await fetchJSON('/api/ohlcv?'+qs({market,symbol,bars})); const rows=data.rows || [];
  if(!rows.length){ $('latestStatus').innerHTML='<span class="bad">No OHLCV rows loaded</span>'; Plotly.purge('ohlcvChart'); return; }
  ...
  $('latestStatus').textContent=`Latest plotted: ${data.latest_close_utc} | rows ${fmtInt(data.row_count)}`;
  $('gapStatus').textContent=`Gaps: ${fmtInt(data.gap_count)} ranges, missing rows ${fmtInt(data.missing_rows_plotted)}`;
}
async function plotTimings(){
  const market=$('market').value, symbol=$('symbol').value, rowsReq=$('timingRows').value || 5000;
  const data=await fetchJSON('/api/timings?'+qs({market,symbol,rows:rowsReq})); const rows=data.rows || [];
  if(!rows.length){ $('timingStatus').innerHTML=`<span class="bad">No timing rows for ${market} ${symbol}</span>`; Plotly.purge('timingChart'); return; }
...
async function pollLatest(){ if(paused) return; const market=$('market').value, symbol=$('symbol').value; if(!market || !symbol) return; try{ const range=await fetchJSON('/api/range?'+qs({market,symbol})); if(range.last_close_time_ms && (!lastSeenCloseMs || range.last_close_time_ms>lastSeenCloseMs)){ lastSeenCloseMs=range.last_close_time_ms; await plotAll(); } else { await plotTimings(); } } catch(e){ console.warn(e); } }
...
static void handle_client(socket_t client) {
    ...
    if (method != "GET") {
        ...
        send_all(client, handle_target(target));
    }
    close_socket(client);
}
