...
// // // // static fs::path LIVE_DELAY_CSV_PATH = LOG_DIR / "live_symbol_delays_microseconds.csv";
// // // // static bool LIVE_DELAY_LOG_ENABLED = true;

// // // // static RunMode RUN_MODE = RunMode::all;

// // // // static std::vector<std::string> SYMBOLS_SPOT_BULK = {"AAVEUSDT", "BTCUSDT", "ETHUSDT", "LTCUSDT", "SOLUSDT", "XRPUSDT"};
...
// // // // static std::vector<std::string> SYMBOLS_SPOT_LIVE_REST = {"AAVEUSDT", "BTCUSDT", "ETHUSDT", "LTCUSDT", "SOLUSDT", "XRPUSDT"};
...
// // // // static bool AUTO_FIND_SYMBOLS_SPOT = false;
// // // // static bool AUTO_FIND_SYMBOLS_UM = false;
// // // // static bool AUTO_FIND_SYMBOLS_CM = false;
...
// // // // static int REST_LOOKBACK_HOURS = 72;
// // // // static int REST_SYMBOL_THREADS = 10;
...
// // // // static int MAX_SYMBOLS_PER_STREAM = 200;
...
// // // // static std::chrono::sys_days BULK_START_DATE{std::chrono::year{2025}/6/1};
...
// // // //     throw std::invalid_argument("unknown market");
// // // // }
// // // // static int rest_limit(Market m) { return m == Market::spot ? 1000 : 1500; }
...
// // // //         << '\n';
// // // // }
// // // // static void append_live_delay_csv(const LiveDelayRecord& r) {
...
// // // //     return resp;
// // // // }
// // // // static std::string safe_get_text(const std::string& url, const std::map<std::string,std::string>& params = {}, int retries = HTTP_RETRIES, const std::string& label = "request_text") {
// // // //     double delay = 1.0;
...
// // // // }
// // // // static json safe_get_json(const std::string& url, const std::map<std::string,std::string>& params = {}, int retries = HTTP_RETRIES, const std::string& label = "request") {
// // // //     return json::parse(safe_get_text(url, params, retries, label));
// // // // }
...
// // // //         if (out.symbol.empty() || out.open_time % INTERVAL_MS != 0 || !is_finite_kline(out)) return std::nullopt;
// // // //         return out;
// // // //     } catch (...) {
// // // //         return std::nullopt;
// // // //     }
...
// // // // static std::vector<Kline> fetch_rest_klines_range(Market market, const std::string& symbol, int64_t start_ms, int64_t end_ms) {
// // // //     std::string sym = upper(symbol);
...
// // // //             {"endTime", std::to_string(end_ms + INTERVAL_MS - 1)}, {"limit", std::to_string(limit)}
// // // //         }, HTTP_RETRIES, "REST " + market_name(market) + " " + sym);
...
// // // //         if (static_cast<int>(data.size()) < limit || next_cursor <= cursor) break;
// // // //         cursor = next_cursor;
...
// // // // static std::pair<std::string,int> update_rest_lookback_for_symbol(Market market, const std::string& symbol, int hours = REST_LOOKBACK_HOURS) {
// // // //     int64_t end_ms = last_closed_minute_open_ms();
// // // //     int64_t start_ms = end_ms - static_cast<int64_t>(hours) * 60 * 60 * 1000 + INTERVAL_MS;
// // // //     auto rows = fetch_rest_klines_range(market, symbol, start_ms, end_ms);
// // // //     int written = write_klines_daily(market, symbol, rows, WriteMode::dedupe_sort, "rest_" + std::to_string(hours) + "h");
// // // //     LOG_INFO("[REST ", market_name(market), " ", symbol, "] fetched=", rows.size(), " wrote=", written);
...
// // // //     return {upper(symbol), static_cast<int>(rows.size())};
// // // // }
// // // // static void gap_fill_after_reconnect(Market market, const std::vector<std::string>& symbols, int64_t gap_since_ms, std::unordered_map<std::string,int64_t>& last_seen_open_ms) {
// // // //     auto clean = clean_symbol_list(symbols);
// // // //     int64_t end_ms = last_closed_minute_open_ms();
// // // //     int64_t fallback_start_ms = floor_minute_ms(gap_since_ms);
// // // //     std::vector<std::tuple<std::string,int64_t,int64_t>> jobs;
// // // //     for (auto& sym : clean) {
// // // //         std::optional<int64_t> disk_latest = read_latest_open_time_ms(market, sym);
// // // //         auto it = last_seen_open_ms.find(sym);
// // // //         std::optional<int64_t> mem_latest = (it == last_seen_open_ms.end()) ? std::nullopt : std::optional<int64_t>{it->second};
// // // //         int64_t start_ms = fallback_start_ms;
// // // //         if (disk_latest || mem_latest) start_ms = std::max(disk_latest.value_or(0), mem_latest.value_or(0)) + INTERVAL_MS;
// // // //         start_ms = floor_minute_ms(start_ms);
// // // //         if (start_ms <= end_ms) jobs.emplace_back(sym, start_ms, end_ms);
// // // //     }
// // // //     if (jobs.empty()) { LOG_INFO("[GAP ", market_name(market), "] no symbol gaps"); return; }
// // // //     std::atomic<size_t> idx{0};
// // // //     std::mutex mu;
// // // //     std::vector<std::thread> workers;
...
// // // //                 try {
// // // //                     auto rows = fetch_rest_klines_range(market, sym, st, en);
// // // //                     write_klines_daily(market, sym, rows, WriteMode::dedupe_sort, "stream_gap_fill");
// // // //                     if (!rows.empty()) {
// // // //                         int64_t latest = std::max_element(rows.begin(), rows.end(), [](auto& a, auto& b){return a.open_time < b.open_time;})->open_time;
...
// // // // static void live_multiplex_loop(Market market, std::vector<std::string> symbols) {
...
// // // //                 gap_fill_after_reconnect(market, symbols, gap_ms, last_seen_open_ms);
...
// // // //                 std::string stream_name = msg.value("stream", "");
// // // //                 json data = msg.contains("data") ? msg["data"] : msg;
// // // //                 if (!data.is_object()) continue;
// // // //                 if (data.value("e", "") == "serverShutdown" || stream_name == "!serverShutdown") {
// // // //                     pending_gap_since_ms = last_ws_message_ms;
// // // //                     throw std::runtime_error("serverShutdown event");
// // // //                 }
// // // //                 if (!data.contains("k") || !data["k"].is_object()) continue;

// // // //                 auto& k = data["k"];

// // // //                 // Local receive timestamp captured immediately after ws.read(buffer).
// // // //                 int64_t received_ms = last_ws_message_ms;

// // // //                 // Binance websocket event time, field "E", if present.
// // // //                 // This is separate from the exact minute boundary.
// // // //                 int64_t exchange_event_ms = 0;
// // // //                 try {
...
// // // //                 if (!k.value("x", false)) continue; // closed candles only
...
// // // //                 // Binance kline close_time "T" is normally open_time + 59,999ms,
// // // //                 // so use open_time + INTERVAL_MS for the true next-minute boundary.
...
// // // //                 auto bin_path = daily_bin_path(market, row->symbol, ms_to_day(row->open_time));

// // // //                 int64_t save_started_us = utc_now_us();
// // // //                 int64_t save_started_ms = save_started_us / 1000;

// // // //                 int written = write_klines_daily(
// // // //                     market,
// // // //                     row->symbol,
// // // //                     {*row},
// // // //                     WriteMode::append_live,
// // // //                     "live"
// // // //                 );

// // // //                 int64_t saved_us = utc_now_us();
// // // //                 int64_t saved_ms = saved_us / 1000;

// // // //                 LiveDelayRecord delay_record;
// // // //                 delay_record.market = market_name(market);
// // // //                 delay_record.symbol = row->symbol;
// // // //                 delay_record.kline_open_time_ms = row->open_time;
// // // //                 delay_record.kline_close_time_ms = row->close_time;
// // // //                 delay_record.scheduled_close_ms = scheduled_close_ms;
// // // //                 delay_record.scheduled_close_us = scheduled_close_us;
// // // //                 delay_record.exchange_event_ms = exchange_event_ms;
// // // //                 delay_record.exchange_event_us = exchange_event_us;
// // // //                 delay_record.received_ms = received_ms;
// // // //                 delay_record.received_us = received_us;
// // // //                 delay_record.save_started_ms = save_started_ms;
// // // //                 delay_record.save_started_us = save_started_us;
// // // //                 delay_record.saved_ms = saved_ms;
// // // //                 delay_record.saved_us = saved_us;

// // // //                 delay_record.scheduled_close_to_receive_us = received_us - scheduled_close_us;
// // // //                 delay_record.exchange_event_to_receive_us =
// // // //                     exchange_event_us > 0 ? received_us - exchange_event_us : 0;
// // // //                 delay_record.receive_to_saved_us = saved_us - received_us;
// // // //                 delay_record.write_call_us = saved_us - save_started_us;
// // // //                 delay_record.scheduled_close_to_saved_us = saved_us - scheduled_close_us;
...
// // // //                 delay_record.rows_written = written;
// // // //                 delay_record.bin_path = bin_path.string();

// // // //                 append_live_delay_csv(delay_record);

// // // //                 last_seen_open_ms[row->symbol] = std::max(last_seen_open_ms[row->symbol], row->open_time);
// // // //             }
// // // //         } catch (const std::exception& e) {
...
// // // //     auto spot_chunks = chunk_symbols(spot);
// // // //     auto um_chunks = chunk_symbols(um);
// // // //     auto cm_chunks = chunk_symbols(cm);
// // // //     std::vector<std::thread> threads;
// // // //     for (auto& c : spot_chunks) threads.emplace_back(live_multiplex_loop, Market::spot, c);
// // // //     for (auto& c : um_chunks) threads.emplace_back(live_multiplex_loop, Market::um, c);
// // // //     for (auto& c : cm_chunks) threads.emplace_back(live_multiplex_loop, Market::cm, c);
// // // //     LOG_INFO("[LIVE] started ", threads.size(), " multiplex connections");
// // // //     for (auto& t : threads) t.join();
// // // // }
...
// // // // Worker threads parse JSON, convert klines, write .bin files, and write delay CSV rows.
// // // // 0 = use all hardware cores reported by std::thread::hardware_concurrency().
...
// // //     auto bin_path = daily_bin_path(job.market, row->symbol, ms_to_day(row->open_time));
...
// // //     append_live_delay_csv(delay_record);
// // //     update_last_seen_open_from_worker(job.stream_state, row->symbol, row->open_time);
// // // }
...
// static int LIVE_BIN_BATCH_FLUSH_US = 20'000;
// static size_t LIVE_BIN_BATCH_FORCE_FLUSH_ROWS = 10'000;
...
//     std::vector<LiveBinWriteJob> pending_;
//     bool shutdown_ = false;

// public:
//     BlockingLiveBinWriteQueue() = default;
...
//         }
//         if (pending_size_for_notify() >= LIVE_BIN_BATCH_FORCE_FLUSH_ROWS) cv_.notify_one();
//         return true;
//     }
...
//             return shutdown_ || pending_.size() >= LIVE_BIN_BATCH_FORCE_FLUSH_ROWS;
//         });
...
//         std::lock_guard<std::mutex> lk(self->mu_);
//         return self->shutdown_ && self->pending_.empty();
//     }
...
// private:
//     size_t pending_size_for_notify() const {
//         auto* self = const_cast<BlockingLiveBinWriteQueue*>(this);
//         std::lock_guard<std::mutex> lk(self->mu_);
//         return self->pending_.size();
//     }
// };
...
// static void live_bin_writer_loop(std::shared_ptr<BlockingLiveBinWriteQueue> bin_queue) {
//     LOG_INFO("[LIVE BIN WRITER] started batch_flush_us=", LIVE_BIN_BATCH_FLUSH_US,
...
//     LOG_INFO("[LIVE BIN WRITER] stopped");
// }
...
//     std::thread live_bin_writer;
//     if (LIVE_BIN_BATCH_WRITES) {
//         live_bin_writer = std::thread(live_bin_writer_loop, live_bin_queue);
//     }
...
//     if (live_bin_writer.joinable()) live_bin_writer.join();
...
static int REST_MAX_WEIGHT_PER_MINUTE = 400;
static bool REST_RATE_LIMIT_SPREAD_REQUESTS = true;
...
static size_t LIVE_BIN_BATCH_FORCE_FLUSH_ROWS = 500;
...
static unsigned int LIVE_BIN_WRITER_THREADS = 8;
static size_t LIVE_BIN_READY_BATCH_QUEUE_MAX = 12;
...
static std::pair<std::string,int> update_rest_lookback_for_symbol(Market market, const std::string& symbol, int rows_back = REST_LOOKBACK_ROWS) {
    ...
    int64_t end_ms = last_closed_minute_open_ms();
    int64_t start_ms = end_ms - static_cast<int64_t>(rows_back - 1) * INTERVAL_MS;

    auto rows = fetch_rest_klines_range(market, symbol, start_ms, end_ms, REST_MAX_CHUNKS_PER_SYMBOL);
    int written = write_klines_daily(market, symbol, rows, WriteMode::dedupe_sort, "rest_" + std::to_string(rows_back) + "rows");

    LOG_INFO("[REST ", market_name(market), " ", symbol,
             "] requested_rows_back=", rows_back,
             " max_chunks=", REST_MAX_CHUNKS_PER_SYMBOL,
...
static void gap_fill_after_reconnect(Market market, const std::vector<std::string>& symbols, int64_t gap_since_ms, std::unordered_map<std::string,int64_t>& last_seen_open_ms) {
    ...
    // Reconnect gap-fill starts at the disconnect minute minus one minute.
    // This intentionally re-requests one already-closed candle for safety; the
    // dedupe/sort writer removes duplicates. It avoids the old behavior of
    // scanning disk/latest and only asks for the exact number of rows needed
    // from the disconnect window.
    int64_t disconnect_start_ms = floor_minute_ms(gap_since_ms) - INTERVAL_MS;
    if (disconnect_start_ms < 0) disconnect_start_ms = 0;

    int64_t reconnect_rows = 0;
    if (disconnect_start_ms <= end_ms) {
        reconnect_rows = ((end_ms - disconnect_start_ms) / INTERVAL_MS) + 1;
    }

    if (reconnect_rows <= 0) {
        LOG_INFO("[GAP ", market_name(market), "] no reconnect gap rows; disconnect_start=",
                 ms_to_utc_string(disconnect_start_ms), " end=", ms_to_utc_string(end_ms));
        return;
    }

    int max_chunks = static_cast<int>((reconnect_rows + rest_limit(market) - 1) / rest_limit(market));
    max_chunks = std::max(1, max_chunks);

    std::vector<std::tuple<std::string,int64_t,int64_t>> jobs;
    jobs.reserve(clean.size());
    for (auto& sym : clean) {
        jobs.emplace_back(sym, disconnect_start_ms, end_ms);
    }

    if (jobs.empty()) {
        LOG_INFO("[GAP ", market_name(market), "] no symbol gaps");
        return;
    }

    LOG_INFO("[GAP ", market_name(market), "] reconnect gap-fill symbols=", jobs.size(),
             " disconnect_start=", ms_to_utc_string(disconnect_start_ms),
             " end=", ms_to_utc_string(end_ms),
             " rows_per_symbol=", reconnect_rows,
             " rest_limit=", rest_limit(market),
             " chunks_per_symbol=", max_chunks,
